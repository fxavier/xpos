...
import com.mz.xpos.model.vo.DataValor;
...
public class Facturas implements Serializable {
	...
	@SuppressWarnings("unchecked")
	public List<Factura> filtrados(FacturaFilter filtro) {
		...
		if (StringUtils.isNotBlank(filtro.getNomeCliente())) {
			// acessamos o nome do cliente associado a factura pelo alias "c", criado anteriormente
			criteria.add(Restrictions.ilike("c.nome", filtro.getNomeCliente(), MatchMode.ANYWHERE));
		}
		
		if (StringUtils.isNotBlank(filtro.getNomeVendedor())) {
			// acessamos o nome do vendedor associado a facura pelo alias "v", criado anteriormente
	...
	public Factura guardar(Factura pedido) {
		return this.manager.merge(pedido);
	}

	public Factura porId(Long id) {
		return this.manager.find(Factura.class, id);
	}
